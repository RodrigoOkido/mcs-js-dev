...
const RecordRoomMessage = require('./messages/recordRoomSet');
...
class MCSResponseClient extends MCSBaseClient {
  ...
  recordRoomSet (roomId, status, params) {
    ...
    var recordRoomMessage =
      new RecordRoomMessage(roomId, status, params);
    if (recordRoomMessage) {
      this.send(recordRoomMessage);
    }
