...
const MediaUpdated = require('./messages/mediaUpdated');
const RecordStatusMessage = require('./messages/recordStatusSet');
...
class MCSResponseClient extends MCSBaseClient {
  ...
  mediaUpdated(mediaId) {
    ...
    var mediaUpdatedMessage = new MediaUpdated(mediaId);
    if (mediaUpdatedMessage) {
      this.send(mediaUpdatedMessage);
    }
  }


  recordStatusSet (roomId, status, params) {
  ...
  var recordStatusMessage =
    new RecordStatusMessage(roomId, status, params);
  if (recordStatusMessage) {
    this.send(recordStatusMessage);
  }
